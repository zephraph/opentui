...
import { MouseEvent, OptimizedBuffer, parseColor, Renderable, RGBA, type ParsedKey, type RenderableOptions, type RenderContext } from ".."
...
export class ScrollAreaRenderable extends Renderable {
    ...
    private set offset(newOffset: number) {
        if (newOffset !== this._offset) {
            ...
            this.viewport.getLayoutNode().yogaNode.setPosition(
                Edge.Top,
                -this._offset
            )
        }
    ...
    private updateThumb() {
        ...
        const pos = clamp(
            (this._maxOffset ? this._offset / this._maxOffset : 0) * (size - thumbSize),
            0,
            size - thumbSize,
        )
    ...
    private updateViewportSize() {
        const node = this.getLayoutNode().yogaNode
        const newSize: Size = {
            width: node.getComputedWidth() - this.verticalThickness,
            height: node.getComputedHeight()
        }
